        ...
        for root, dirs, files in os.walk(args[0]):
            for file in files:
                ...
                if (valid_regex.search(file) and os.path.getsize(filename) > 60):
                    ...
                    yield data, filename 
class SearchNasty:
    """Generator that searches a given file for evaulating expressions"""       
    def search_file_regex(self, top_ten):
            #valid_regex = re.compile('eval')
            for filename in top_ten:
                for file,rank in filename:
                    if (os.path.getsize(file) > 60):
                        try:
                            ...
                            #
                            #
                            # Incomplete regex and hit_count doesnt work
                            #
                            #
                            valid_regex = re.compile('(eval|base64_decode|python_eval)')
...
#
#
# Calling the nasty beast search function
#
    nastybeast = SearchNasty()
    nastybeast.search_file_regex(top_ten)
